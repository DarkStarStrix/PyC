...
function setLink(element, asset, fallback, fallbackText) {
    if (!element) {
        return;
    }
    if (asset) {
...
function renderAdapterRows(tbodyId, rows) {
...
function renderDistributedKpis(latest) {
    const grid = byId("dist-kpi-grid");
    if (!grid || !latest) {
        return;
    }
    grid.innerHTML =
...
async function loadRelease() {
    ...
    const linuxLink = byId("download-linux");
    const macosLink = byId("download-macos");
    const windowsLink = byId("download-windows");
...
async function loadPublishedResults() {
    ...
    try {
        const summary = await fetchJsonWithFallback(
            [
        ...
        if (status) {
            status.textContent = `Latest baseline run: ${latestRun} | CPU adapters: ${cpuRows.length} | GPU adapters: ${gpuRows.length}`;
        }
    } catch (_error) {
...
function renderBaselineCharts() {
    setImageWithFallback(byId("latest-cpu-svg"), "./results/artifacts/latest/latest_cpu.svg", `${RAW_RESULTS_BASE}/artifacts/latest/latest_cpu.svg`);
    setImageWithFallback(byId("latest-gpu-svg"), "./results/artifacts/latest/latest_gpu.svg", `${RAW_RESULTS_BASE}/artifacts/latest/latest_gpu.svg`);
}

async function loadDistributedInsights() {
    const status = byId("distributed-status");
    try {
        ...
        setImageWithFallback(byId("latest-dist-summary-main"), latest.published?.summary_svg ?? null, "./results/artifacts/images/latest_distributed_pipeline.svg");
        setImageWithFallback(byId("latest-dist-throughput-main"), payload.visuals?.throughput_svg ?? null, "./results/artifacts/images/latest_distributed_throughput.svg");
        setImageWithFallback(byId("latest-dist-pipeline-main"), payload.visuals?.pipeline_svg ?? null, "./results/artifacts/images/latest_distributed_pipeline.svg");
    } catch (_error) {
        if (status) {
            status.textContent = "Distributed training insights unavailable.";
        }
