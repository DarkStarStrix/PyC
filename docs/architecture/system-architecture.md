...
This document explains how PyC is built and how execution flows from compile-time planning to runtime dispatch. It is intentionally implementation-oriented and aligned with the current code in `src/core/`, `src/compiler/`, and `include/pyc/`.
...
```mermaid
flowchart TD
    A[CMakeLists.txt] --> B[src/core/C_Files + src/core/Header_Files]
    A --> C[src/compiler/ir]
    A --> D[src/compiler/passes]
    A --> E[src/compiler/runtime]
    A --> F[src/compiler/compiler_api.c]
    G[include/pyc/*.h] --> C
    G --> D
    G --> E
    G --> F
...
|---|---|---|---|
| IR | `include/pyc/ir.h` | `src/compiler/ir/ir.c` | op graph model, verification, deterministic serialization |
| Pass pipeline | `include/pyc/pass_manager.h` | `src/compiler/passes/pass_manager.c` | canonicalization, shape inference, fusion, liveness, graph-break analysis |
| Allocator planner | `include/pyc/runtime_allocator.h` | `src/compiler/runtime/runtime_allocator.c` | reuse planning, pressure scoring, rematerialization counts |
| Kernel registry | `include/pyc/kernel_registry.h` | `src/compiler/runtime/kernel_registry.c` | kernel candidate registration, scoring, selection, autotune stats |
| Runtime rails | `include/pyc/runtime_control.h` | `src/compiler/runtime/runtime_control.c` | objective-mode switching and rollback policy |
| Compiler API | `include/pyc/compiler_api.h` | `src/compiler/compiler_api.c` | compile/run lifecycle, cache, decision logs, output stats |
| CUDA dispatch | `include/pyc/cuda_backend.h` | `src/compiler/runtime/cuda_backend.c` | native CUDA path, graph replay, reason-coded fallback |
